...
function activate(context) {
  const mustache_new_file = vscode.commands.registerCommand('mustache.new.file', (args) => {
    mustache(vscode, fs, path, args);
  });
  const mustache_change_delimiters = vscode.commands.registerCommand('mustache.change.delimiters', () => {
    delimiters(vscode);
  });
